...
5. Een **pure beslislaag** toepast (zie hieronder): tags, genres, leeftijd, schijfdruk, waarschuwingsvenster.
...
| Root folders | GET | `/api/v3/rootfolder` | Eerste entry of gekozen pad voor **schijfgebruik** (zie hieronder). |
...
- Haal **root folder path(s)** via GET `/api/v3/rootfolder` (gebruik bv. `[0].path` zoals nu).
- Gebruik **`psutil.disk_usage(path)`** om **percentage gebruikt** te bepalen.
- Vergelijk met config **`REMOVE_MOVIES_DISK_PERCENTAGE`** (float). `isFull = percent >= drempel`.

**Belangrijk gedrag (bewust):** de **hele filmloop** (evaluatie, logging, verwijderingen) draait alleen als **media aanwezig is én `isFull`**. Onder de drempel wordt de bibliotheek **niet** per film doorlopen (ook geen unwanted-genre verwijdering in die run). Documenteer dit in config-commentaar.
...
5. Als `now - download_date >= remove_after_days` **en** `is_full` **en** niet (maand in `months_no_exclusion` **of** tag in `tags_no_exclusion_ids`) → **removed** met `add_import_exclusion = not (monthfound or exclusiontagsfound)`.
6. Anders → **active**.
...
- Gedrag wijzigen van “alleen bij volle schijf” tenzij expliciet gevraagd.
